...
export const Sidebar: React.FC<SidebarProps> = ({ activeTab, onSelectTab }) => {
  ...
  return (
    <aside className="w-64 bg-white dark:bg-[#0D121F] border-r border-slate-200 dark:border-slate-800 flex flex-col justify-between shrink-0 select-none transition-colors">
      <div className="py-3 px-2 space-y-1 overflow-y-auto">
        <div className="px-3 py-1.5 text-[10px] uppercase tracking-wider font-semibold text-slate-400 dark:text-slate-500">
          Navegação Principal
        </div>
        {menuItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              {item.badge && (
                <span className="text-[9px] font-mono px-1.5 py-0.2 rounded bg-indigo-100 dark:bg-indigo-500/20 text-indigo-700 dark:text-indigo-300 border border-indigo-300 dark:border-indigo-500/30">
                  {item.badge}
                </span>
              )}
            </button>
      ...
      <div className="p-3 border-t border-slate-200 dark:border-slate-800/80 bg-slate-50 dark:bg-slate-950/40">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            <div>
              <p className="text-xs font-semibold text-slate-800 dark:text-slate-200">Rotina User</p>
              <p className="text-[10px] text-slate-500">Offline-First Engine</p>
            </div>
          </div>
          <span className="w-2 h-2 rounded-full bg-emerald-500" title="Motor Local Funcional"></span>
        </div>
      </div>
    </aside>
  );
